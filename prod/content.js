...
function displayOverlayResults(results, container) {
  ...
  Object.keys(resultsByUrl).forEach((url, urlIndex) => {
    ...
    const dropdownId = `dropdown-${urlIndex}`;
         ...
         // Create dropdown header - same for both current and linked pages
     html += `
       <div class="dropdown-header" data-dropdown-id="${dropdownId}" data-action="toggle" style="
         margin-bottom: 8px; 
         padding: 10px 12px; 
         background: ${isCurrentPage ? '#d4edda' : '#fff3cd'}; 
         border-radius: 4px; 
         font-size: 12px; 
         font-weight: 600; 
         color: ${isCurrentPage ? '#155724' : '#856404'};
         cursor: pointer;
         display: flex;
         justify-content: space-between;
         align-items: center;
         border: 1px solid ${isCurrentPage ? '#c3e6cb' : '#ffeaa7'};
         transition: background-color 0.2s;
       ">
         <span>${pageIndicator} (${occurrences.length} results)${!isCurrentPage ? ' - Click words to open page' : ''}</span>
    ...
    `;
     ...
     occurrences.forEach((occurrence, index) => {
       ...
       if (isCurrentPage) {
         // Current page results - highlight on current page
         html += `
         ...
         // Linked page results - open page and highlight
         html += `
           <div style="margin-bottom: 8px; padding: 10px; border: 1px solid #eee; border-radius: 4px; cursor: pointer; background: white; transition: background-color 0.2s;" 
                onclick="window.postMessage({type: 'betterFind-open-linked-page', url: '${url}', searchword: '${searchword}'}, '*')"
                onmouseover="this.style.background='#f8f9fa'" 
                onmouseout="this.style.background='white'">
             <div style="font-weight: 600; margin-bottom: 4px; font-size: 12px;">Match ${occurrence.originalIndex + 1} <span style="color: #007bff; font-size: 10px;">(Click to open page)</span></div>
             <div style="font-size: 11px; color: #666; margin-bottom: 4px;">Position: ${occurrence.position}</div>
             <div style="font-size: 11px; margin-bottom: 4px;">
               <strong>Context:</strong> ${context} <span style="background: #ffff00; padding: 1px 2px; border-radius: 2px;">${searchword}</span> ${afterContext}
             </div>
  ...
  container.innerHTML = html;
   ...
   dropdownHeaders.forEach(header => {
     ...
     const isCurrentPage = header.textContent.includes('Current Page');
     
     // Add hover effects based on page type
     if (isCurrentPage) {
     ...
     // Add click handler - only toggle dropdown
